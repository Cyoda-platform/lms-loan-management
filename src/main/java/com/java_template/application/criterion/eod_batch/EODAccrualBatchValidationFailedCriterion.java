...
/**
 * ABOUTME: Negative criterion that succeeds when EOD batch validation fails.
 * Used to route to error state when validation fails.
 */
@Component
public class EODAccrualBatchValidationFailedCriterion extends EODAccrualBatchValidationCriterion {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    private final CriterionSerializer serializer;
    private final String className = this.getClass().getSimpleName();

    public EODAccrualBatchValidationFailedCriterion(SerializerFactory serializerFactory, EntityService entityService) {
    ...
    public EntityCriteriaCalculationResponse check(CyodaEventContext<EntityCriteriaCalculationRequest> context) {
        ...
        logger.debug("Checking EODAccrualBatchValidationFailed (negative criterion) for request: {}", request.getId());

        return serializer.withRequest(request)
            .evaluateEntity(EODAccrualBatch.class, this::validateEntityFailed)
    ...
    private EvaluationOutcome validateEntityFailed(CriterionSerializer.CriterionEntityEvaluationContext<EODAccrualBatch> context) {
