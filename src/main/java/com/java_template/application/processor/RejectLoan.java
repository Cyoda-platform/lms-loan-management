...
@Component
public class RejectLoan implements CyodaProcessor {
    private static final Logger logger = LoggerFactory.getLogger(RejectLoan.class);
    private final String className = this.getClass().getSimpleName();
    private final ProcessorSerializer serializer;

    public RejectLoan(SerializerFactory serializerFactory) {
    ...
    private EntityWithMetadata<Loan> process(ProcessorSerializer.ProcessorEntityResponseExecutionContext<Loan> context) {
        ...
        logger.info("Loan {} rejected", entityWithMetadata.entity().getLoanId());
